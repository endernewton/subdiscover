function [bs, count] = gdetect_write(featurename, pyra, model, bs, trees, from_pos, ...
...
count = 0;
if ~isempty(bs)
  count = writefeatures(pyra, model, trees, from_pos, dataid, maxsize, maxnum, featurename);
  % truncate boxes
  bs(count+1:end,:) = [];
end
...
function count = writefeatures(pyra, model, trees, from_pos, ...
                               dataid, maxsize, maxnum, featurename)
if from_pos
  is_belief = 1;
else
  is_belief = 0;
...
function ex = addfilterfeat(model, ex, x, y, padx, pady, ds, fi, feat, featurename)
% model object model
