function Scores = imgdetectSCORE(im, models, threshs, featurename, options)
...
if isfield(options,'bFlipTest')
    bFlipTest = options.bFlipTest;
end

maxNumScoresPerImg = 1000;
if isfield(options,'maxNumScores')
    maxNumScoresPerImg = options.maxNumScores;
end
...
r = bsxfun(@plus, r, bs);
clear bs
Scores = nan(N,maxNumScoresPerImg,'single');
% Score = cell(N,1);
...
%     end
    ...
    if lg > maxNumScoresPerImg
        indres = randperm(lg);
        indres = indres(1:maxNumScoresPerImg);
        goods = goods(indres);
        Scores(exid,:) = result(goods);
    else
        Scores(exid,1:lg) = result(goods);
    end
