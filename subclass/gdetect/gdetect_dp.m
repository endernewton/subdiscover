function model = gdetect_dp(pyra, model, featurename)
...
model = filter_responses(model, pyra, featurename);
...
function model = filter_responses(model, pyra, featurename)
% model    object model
% pyra     feature pyramid

% gather filters for computing match quality responses
filters = cell(model.numfilters, 1);
if size(pyra.feat{1},3) == 32
    for i = 1:model.numfilters
        filters{i} = model_get_block(model, model.filters(i),featurename);
    end
else
    for i = 1:model.numfilters
        filters{i} = single(model_get_block(model, model.filters(i),featurename));
    end
end
...
%     r = myfastconv(pyra.feat{level}, filters, 1, length(filters));
  end 
