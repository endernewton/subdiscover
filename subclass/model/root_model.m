function model = root_model(cls, pos, note, sbin, interval, featurename, sz)
...
featureSet=lsvmFeature(featurename);

% get an empty model
model = model_create(cls, note, interval, sbin, featureSet);
...
% add root filter
[model, symbol] = model_add_terminal(model, featurename, 'w', zeros([sz featureSet.dim]));
