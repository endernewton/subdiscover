function objGetDetectionsWrapper(iid, options)
% Get the LSVM detections for each category, and count how many are detected
% -------------------------------------------------------------------------
...
offset = 0;
if isfield(options,'offsetGet')
    offset = options.offsetGet;
end
...
for d=1:length(datasets)
    ...
    for i=1:length(classes)
        ...
        clustername = [detpath,'dpmclusters.mat'];
        clusterfile = [detpath,'clusters.mat'];
        
        if ~fileExists(modelSetname) || ~fileExists(boxSetname) || fileExists(clustername)
            continue;
        else
            clear models;
            disp('Loading models...');
        ...
        save(clustername,'detections','detectionimages','clusters','leftright');
        save(clusterfile,'detsizes','-append');
    end
