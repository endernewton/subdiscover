function model = lsvmTrain(model, pos, neg, warp, randneg, iter, ...
                       negiter, max_num_examples, fg_overlap, ...
                       num_fp, cont, tag, C, cache_byte_limit, cacheDir, featurename)
...
lbfgs.options.verbose = 2;
lbfgs.options.maxIter = 1000;
lbfgs.options.optTol = 0.000001;

interval_fg = 5;
interval_bg = 4;

% Feature_opts=lsvmFeature(featurename);
...
if ~cont
  ...
  max_num = ceil(bytelimit / (single_byte_size*max_dim));
...
for t = 1:iter
  ...
  if ~cont || t > 1
    ...
    if warp
      [num_entries_added, num_examples_added] ...
          = poswarp(t, model, pos, featurename);
      ...
      [num_entries_added, num_examples_added, fusage, component_usage, scores] ...
          = poslatent(t, iter, model, pos, fg_overlap, num_fp, interval_fg, featurename);
      th = toc(th);
  ...
  for tneg = 1:negiter
    ...
    if datamine
      ...
      if randneg
        [num_entries_added, num_examples_added] ...
          = negrandom(t, model, neg, max_num_examples-num_examples, featurename);
        num_entries = num_entries + num_entries_added;
        num_examples = num_examples + num_examples_added;
        fusage = num_examples_added;
      else
        ...
        [num_entries_added, num_examples_added, ...
         negpos, fusage, scores, complete] ...
            = neghard(tneg, negiter, model, neg, bytelimit, ...
                      negpos, max_num_examples-num_examples,interval_bg, featurename);
        th = toc(th);
    ...
    %{{{ Optimize the convex slave problem
      ...
      th = tic;
      w = minConf_TMP(obj_func, w, lb, ub, lbfgs.options);
      th = toc(th);
      ...
      save([cacheDir '/' model_name], 'model');
...
function [num_entries, num_examples] = poswarp(t, model, pos, featurename)
...
for i = 1:numpos
  ...
  % get example
  im = warped{i};
  feat=featuresWrapper(double(im), model.sbin, featurename);
...
for i = 1:batchsize:numpos
  ...
  parfor k = 1:thisbatchsize
    ...
    [pyra, model_dp] = gdetect_pos_prepare(im, model, boxes, fg_overlap, featurename);
    data(k).pyra = pyra;
  ...
  for k = 1:thisbatchsize
    ...
    for b = 1:length(pos(j).dataids)
      ...
      bs = gdetect_write(featurename, data(k).pyra, model, data(k).boxdata{b}.bs, ...
...
function [num_entries, num_examples, j, fusage, scores, complete] ...
  = neghard(t, negiter, model, neg, maxsize, negpos, max_num_examples, interval_bg, featurename)
% conf = voc_config();
model.interval = interval_bg;
fusage = zeros(model.numfilters, 1);
numneg = length(neg);
num_entries = 0;
num_examples = 0;
...
for i = 1:batchsize:numneg
  ...
  parfor k = 1:thisbatchsize
    ...
    [ds, bs, trees] = gdetect(pyra, model, -1.002, det_limit, featurename);
    data{k}.bs = bs;
    data{k}.pyra = pyra;
    data{k}.trees = trees;
  end
  ...
  for k = 1:thisbatchsize
    j = inds(i+k-1);
    dataid = neg(j).dataid;
    bs = gdetect_write(featurename, data{k}.pyra, model, data{k}.bs, data{k}.trees, ...
...
function [num_entries, num_examples] ...
  = negrandom(t, model, neg, maxnum, featurename)
numneg = length(neg);
