function [A, B] = lrsplit(model, pos, fname)
...
dim = numel(featuresWrapper(double(warped{1}), model.sbin, fname));
F = zeros(numpos, dim);
for i = 1:numpos
  tic_toc_print('%s %s: lrsplit features: %d/%d\n', ...
                procid(), model.class, i, length(warped));
  F(i,:) = reshape(featuresWrapper(double(warped{i}), model.sbin, fname), [1 dim]);
end
