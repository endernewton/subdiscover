function pyra = lsvmfeatpyramidBLOCK(im, models, fname, padx, pady)
...
padx = max(max(sizesx-2),0);
pady = max(max(sizesy-2),0);

padx = ceil(padx/2)*2;
pady = ceil(pady/2)*2;

extra_interval = 0;
if models{1}.features.extra_octave
  extra_interval = models{1}.interval;
end
...
%   pyra.feat{i+extra_interval+interval} = features(scaled, sbin);
  ...
  for j = i+interval:interval:max_scale
    ...
    pyra.feat{j+extra_interval+interval} = featuresWrapper(scaled, sbin, fname);
...
td = models{1}.features.truncation_dim;
